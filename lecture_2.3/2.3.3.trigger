trigger ValidationRulesFlight on Flight__c(before insert) {
	...
	for (Flight__c registration: Trigger.new) {
		Tourist__c touristFlight = [SELECT Id, Age__c FROM Tourist__c WHERE Id = :registration.Tourist__c];
		Trip__c tripFlight = [SELECT Id, Minimum_Age__c, Start_Date__c FROM Trip__c WHERE Id = :registration.Trip__c];
